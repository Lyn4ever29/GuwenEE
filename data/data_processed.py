...
def split_train_dev(rate):
    """
    Splitting the original dataset into a test set and a training set.

    Args:
        rate:  Training set and test set ratio.

    Returns:

    """
    dataset=[]
    with open("../corpus/GuwenEE.json","r",encoding="utf-8") as f:
        lines = f.readlines()
        for line in lines:
    ...
    with open("./original/train.json", 'w', encoding="utf-8") as f:
        for data in dataset[:m]:
            json.dump(data, f, ensure_ascii=False)
            f.write("\n")

    with open("./original/dev.json", 'w', encoding="utf-8") as f:
        for data in dataset[m:]:
            json.dump(data, f, ensure_ascii=False)
            f.write("\n")
...
def chinese_tokenizer(input_text: str,
    ...
    if tokenizer == "hanlp":
        raise NotImplementedError
    if tokenizer == "jiayan":
        lm = load_lm('../tools/jiayan.klm')
        tokenizer = CharHMMTokenizer(lm)
        # print(list(tokenizer.tokenize(text)))
        # tokenizer = WordNgramTokenizer()
        token_list =list(tokenizer.tokenize(input_text))
...
def convert_duee_to_unified(data_path: str,
                            ...
                            tokenizer: Optional[str] = "jiayan") -> List[Dict[str, Union[str, List[Dict]]]]:
    ...
    # data_path = './processed'.join(data_path[:-1]).split('/data/original'))
    # data_path_split = data_path.split("/")
    # file_type =data_path_split[-1].split(".")[0]
    # data_path_split[:-1].insert(-1, file_type)
    data_path=data_path.replace("original","processed")
    # os.makedirs("/".join(data_path[:-1]), exist_ok=True)


    if dump:
        with jsonlines.open(data_path.replace('dev', 'valid').replace('test2', 'test').replace('.json', '.unified.jsonl'), "w") as f:
...
if __name__ == "__main__":
    arg_parser = argparse.ArgumentParser(description="GuwenEE")
    arg_parser.add_argument("--train_rate", type=str, default=0.75)
    arg_parser.add_argument("--data_dir", type=str, default="./original")
    arg_parser.add_argument("--save_dir", type=str, default="./processed")
    args = arg_parser.parse_args()

    os.makedirs(args.data_dir, exist_ok=True)
    os.makedirs(args.save_dir, exist_ok=True)
    split_train_dev(args.train_rate)
